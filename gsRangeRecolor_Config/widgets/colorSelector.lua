...
local L = Addon.L
local gsRangeRecolor = _G.gsRangeRecolor
...
function ColorSelector:New(state, parent)
    ...
    for i, color in ipairs(ColorChannels) do
        ...
        slider.SetSavedValue = function(_, value)
            gsRangeRecolor.sets[state][i] = math.floor(value + 0.5) / 100

            local r, g, b, a = gsRangeRecolor:GetColor(state)
            previewIcon:SetVertexColor(r, g, b, a)

            gsRangeRecolor:RequestUpdate()
        end

        slider.GetSavedValue = function()
            return gsRangeRecolor.sets[state][i] * 100
        end
    ...
    desaturate:SetScript("OnShow", function(checkbox)
        local desaturated = gsRangeRecolor.sets[state].desaturate and true
    ...
    desaturate:SetScript("OnClick", function(checkbox)
        ...
        gsRangeRecolor.sets[state].desaturate = desaturated
        gsRangeRecolor:RequestUpdate()
    end)
